...
def calc_smm(coupon_rate, market_rates):
    """
    Calculates the single monthly mortality rate (smm) given the coupon rate
    and market rate. Assumes a simple S curve function.
...
def calc_cpr(smm):
    """
    Calculates the conditional prepayment rate from the single
    ...
    -------
    cpr: float
        A conditional prepayment rate (CPR) indicates a loan prepayment rate
        at which a pool of loans, such as a mortgage backed security's (MBS),
...
class mbs(object):
    ...
    ----------
    mortgage: mortgage
        Instance of class mortgage that will be pooled together for the MBS.
    smm: array_like
        Single monthly mortality is the amount of principal on
        mortgage-backed securities that is prepaid in a given month.
    ...
    def pool_mortgage(self):
        """
        Creates a pooled mortgage using the mortgage instance and pool factor.
        Uses the instance's amortization schedule and the SMM to pull out
        prepaid principal.
        """
